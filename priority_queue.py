...
def MinHeapify(x, frontier=frontier, node_positions=node_positions):
    if len(frontier) > 0:
        ...
        if left(x) < len(frontier):
            if getPriority(left(x)) < getPriority(lowest):
                lowest=left(x)
            
            if right(x) < len(frontier):
                if getPriority(right(x)) < getPriority(lowest):
                    lowest=right(x)
        
        if lowest != x:
            temp = frontier[x]
            temp_pos = node_positions[frontier[x].id]

            frontier[x] = frontier[lowest]
            node_positions[frontier[x].id] = node_positions[frontier[lowest].id]

            frontier[lowest] = temp
            node_positions[frontier[lowest].id] = temp_pos
...
def ChangeKey(id, path_cost, frontier=frontier, node_positions=node_positions):
    ...
    #New path_cost should always be less
